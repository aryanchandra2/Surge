    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
				...
				float4 color : COLOR;
				#if OUTTEX_ON
				float2 uvOutTex : TEXCOORD1;
				#endif
				#if OUTDIST_ON
				float2 uvOutDistTex : TEXCOORD2;
				#endif
				#if DISTORT_ON
				float2 uvDistTex : TEXCOORD3;
				#endif
				#if FADE_ON
				float2 uvFadeTex1 : TEXCOORD4;
				float2 uvFadeTex2 : TEXCOORD5;
				#endif
				#if FOG_ON
				UNITY_FOG_COORDS(6)
				#endif
			...
			#if ATLAS_ON
			float _MinXUV, _MaxXUV, _MinYUV, _MaxYUV;
			#endif
			...
			#if CLIPPING_ON
			float _ClipUvLeft, _ClipUvRight, _ClipUvUp, _ClipUvDown;
			#endif

			#if TWISTUV_ON
			float _TwistUvAmount, _TwistUvPosX, _TwistUvPosY, _TwistUvRadius;
			#endif

			#if ROTATEUV_ON
			float _RotateUvAmount;
			#endif
			...
			#if DOODLE_ON
			float _HandDrawnAmount, _HandDrawnSpeed;
			#endif
			...
			#if ROUNDWAVEUV_ON
			float _RoundWaveStrength, _RoundWaveSpeed;
			#endif
			...
			float4 _FadeBurnColor, _FadeTex_ST, _FadeBurnTex_ST;
			float _FadeAmount, _FadeBurnWidth, _FadeBurnTransition,_FadeBurnGlow;
			#endif
			...
			#if WIND_ON
			float _GrassSpeed, _GrassWind, _GrassManualAnim, _GrassRadialBend;
			#endif
			...
			float4 _ColorSwapRed, _ColorSwapGreen, _ColorSwapBlue;
			float _ColorSwapRedLuminosity, _ColorSwapGreenLuminosity, _ColorSwapBlueLuminosity;
			#endif

			#if HSV_ON
			float _HsvShift, _HsvSaturation, _HsvBright;
			#endif
			...
			#if GREYSCALE_ON
			float _GreyscaleLuminosity;
			float4 _GreyscaleTintColor;
			#endif

			#if POSTERIZE_ON
			float _PosterizeNumColors, _PosterizeGamma;
			#endif
			...
			#if MOTIONBLUR_ON
			float _MotionBlurAngle, _MotionBlurDist;
			#endif

			#if GHOST_ON
			float _GhostColorBoost, _GhostTransparency;
			#endif
			...
			#if HOLOGRAM_ON
			float _HologramStripesAmount, _HologramMinAlpha, _HologramUnmodAmount, _HologramStripesSpeed, _HologramMaxAlpha;
			float4 _HologramStripeColor;
			#endif

			#if CHROMABERR_ON
			float _ChromAberrAmount, _ChromAberrAlpha;
			#endif

			#if GLITCH_ON
			float _GlitchAmount, _GlitchSize;
			#endif

			#if FLICKER_ON
			float _FlickerFreq, _FlickerPercent, _FlickerAlpha;
			#endif
			...
			float4 _ShineColor;
			float _ShineLocation, _ShineRotate, _ShineWidth, _ShineGlow;
			#endif
			...
			#if ALPHAROUND_ON
			float _AlphaRoundThreshold;
			#endif

			#if CHANGECOLOR_ON
			float4 _ColorChangeNewCol, _ColorChangeTarget;
			float _ColorChangeTolerance, _ColorChangeLuminosity;
			#endif
			#if CHANGECOLOR2_ON
			float4 _ColorChangeNewCol2, _ColorChangeTarget2;
			float _ColorChangeTolerance2;
			#endif
			#if CHANGECOLOR3_ON
			float4 _ColorChangeNewCol3, _ColorChangeTarget3;
			float _ColorChangeTolerance3;
			#endif
            ...
            v2f vert (appdata v)
            {
				...
				#if BILBOARDY_ON
				camUp = mul((float3x3)unity_CameraToWorld, float3(0,1,0));
				#endif
				...
				#if ATLAS_ON
				center = float2((_MaxXUV + _MinXUV) / 2.0, (_MaxYUV + _MinYUV) / 2.0);
				#endif
				...
				#if ROTATEUV_ON
				float2 uvC = v.uv;
				float cosAngle = cos(_RotateUvAmount);
				float sinAngle = sin(_RotateUvAmount);
			...
			float3 GetPixel(in int offsetX, in int offsetY, float2 uv, sampler2D tex)
			{
				return tex2D(tex, (uv + float2(offsetX * _MainTex_TexelSize.x, offsetY * _MainTex_TexelSize.y))).rgb;
			}
            ...
            float4 frag (v2f i) : SV_Target
            {
				...
				#if ATLAS_ON
				center = float2((_MaxXUV + _MinXUV) / 2.0, (_MaxYUV + _MinYUV) / 2.0);
				uvRect = float2((i.uv.x - _MinXUV) / (_MaxXUV - _MinXUV), (i.uv.y - _MinYUV) / (_MaxYUV - _MinYUV));
				#endif
				float2 centerTiled = float2(center.x *  _MainTex_ST.x, center.y *  _MainTex_ST.y);

				#if CLIPPING_ON
				float2 tiledUv = float2(i.uv.x / _MainTex_ST.x, i.uv.y / _MainTex_ST.y);
				#if ATLAS_ON
				tiledUv = float2((tiledUv.x - _MinXUV) / (_MaxXUV - _MinXUV), (tiledUv.y - _MinYUV) / (_MaxYUV - _MinYUV));
				#endif
				...
				i.uv = float2(_MinXUV + ((_MaxXUV - _MinXUV) * (abs(((_Time.y + _RandomSeed) * _TextureScrollXSpeed) + uvRect.x) % 1)),
				_MinYUV + ((_MaxYUV - _MinYUV) * (abs(((_Time.y + _RandomSeed) * _TextureScrollYSpeed) + uvRect.y) % 1)));
				#endif

				#if OFFSETUV_ON
				#if ATLAS_ON
				i.uv = float2(_MinXUV + ((_MaxXUV - _MinXUV) * (abs((_OffsetUvX + uvRect.x) % 1))),
				...
				#if POLARUV_ON
				i.uv = float2(atan2(i.uv.y, i.uv.x) / (2.0f * 3.141592653589f), length(i.uv));
			    i.uv *= _MainTex_ST.xy;
				#endif
				...
				#endif
				float2 tempUv = i.uv - float2(_TwistUvPosX *  _MainTex_ST.x, _TwistUvPosY *  _MainTex_ST.y);
				...
				tempUv += float2(_TwistUvPosX *  _MainTex_ST.x, _TwistUvPosY *  _MainTex_ST.y);
				i.uv = tempUv;
				#endif

				#if FISHEYE_ON
				float bind = length(centerTiled);
				float2 dF = i.uv - centerTiled;
				...
				#if SHAKEUV_ON
				float xShake = sin((_Time + _RandomSeed) * _ShakeUvSpeed * 500 * _ShakeUvX);
				float yShake = sin((_Time + _RandomSeed) * _ShakeUvSpeed * 500 * _ShakeUvY);
				i.uv += float2(xShake * 0.01, yShake * 0.01);
				#endif
				...
				#if ATLAS_ON
				i.uvDistTex = float2((i.uvDistTex.x - _MinXUV) / (_MaxXUV - _MinXUV), (i.uvDistTex.y - _MinYUV) / (_MaxYUV - _MinYUV));
				#endif
				float distortAmnt = (tex2D(_DistortTex, i.uvDistTex).r - 0.5) * 0.2 * _DistortAmount;
				i.uv.x += distortAmnt;
				i.uv.y += distortAmnt;
				#endif
				...
				float angWave = (sqrt(dot(uvWave, uvWave)) * _WaveAmount) - (((_Time.y + _RandomSeed) *  _WaveSpeed) % 360.0);
				...
				#if WIND_ON
				float windOffset = sin((_Time + _RandomSeed) * _GrassSpeed * 10);
				...
				#endif
				float2 delta = i.uv - windCenter;
				float delta2 = dot(delta.xy, delta.xy);
				float2 delta_offset = delta2 * windOffset;
				i.uv = i.uv + float2(delta.y, -delta.x) * delta_offset * _GrassRadialBend;
				#endif
				...
				#if GLITCH_ON
				float2 uvGlitch = uvRect;
				uvGlitch.y -= 0.5;
				float lineNoise = pow(rand2(floor(uvGlitch * float2(24., 19.) * _GlitchSize) * 4.0, _RandomSeed), 3.0) * _GlitchAmount
					* pow(rand2(floor(uvGlitch * float2(38., 14.) * _GlitchSize) * 4.0, _RandomSeed), 3.0);
				col = tex2D(_MainTex, i.uv + float2(lineNoise * 0.02 * rand2(float2(2.0, 1), _RandomSeed), 0)) * i.color;
				#endif

				#if CHROMABERR_ON
				float4 r = tex2D(_MainTex, i.uv + float2(_ChromAberrAmount/10, 0)) * i.color;
				float4 b = tex2D(_MainTex, i.uv + float2(-_ChromAberrAmount/10, 0)) * i.color;
				col = float4(r.r * r.a, col.g, b.b * b.a, max(max(r.a, b.a) * _ChromAberrAlpha, col.a));
				#endif
				...
				col.rgb += tex2D(_MainTex, i.uv + rot(float2(-_MotionBlurDist, -_MotionBlurDist)));
				col.rgb += tex2D(_MainTex, i.uv + rot(float2(-_MotionBlurDist * 2, -_MotionBlurDist * 2)));
				col.rgb += tex2D(_MainTex, i.uv + rot(float2(-_MotionBlurDist * 3, -_MotionBlurDist * 3)));
				col.rgb += tex2D(_MainTex, i.uv + rot(float2(-_MotionBlurDist * 4, -_MotionBlurDist * 4)));
				...
				col.rgb = float3(luminance, luminance, luminance) * _GreyscaleTintColor;
				#endif
				...
				#if GRADIENT_ON
				float2 tiledUvGrad = float2(uvRect.x / _MainTex_ST.x, uvRect.y / _MainTex_ST.y);
				#if GRADIENT2COL_ON
				_GradTopRightCol = _GradTopLeftCol;
				_GradBotRightCol = _GradBotLeftCol;
				#endif
				#if RADIALGRADIENT_ON
				float radialDist = 1 - length(tiledUvGrad - float2(0.5, 0.5));
				radialDist *= (_MainTex_TexelSize.w / _MainTex_TexelSize.z);
				radialDist = saturate(_GradBoostX * radialDist);
				float4 gradientResult = lerp(_GradTopLeftCol, _GradBotLeftCol, radialDist);
				#else
				float gradXLerpFactor = saturate(pow(tiledUvGrad.x, _GradBoostX));
				...
				#if COLORSWAP_ON
				luminance = 0.3 * col.r + 0.59 * col.g + 0.11 * col.b;
				float4 swapMask = tex2D(_ColorSwapTex, i.uv);
				swapMask.rgb *= swapMask.a;
				float3 redSwap = _ColorSwapRed * swapMask.r * col.a * saturate(luminance + _ColorSwapRedLuminosity);
				float3 greenSwap = _ColorSwapGreen * swapMask.g * col.a * saturate(luminance + _ColorSwapGreenLuminosity);
				float3 blueSwap = _ColorSwapBlue * swapMask.b * col.a * saturate(luminance + _ColorSwapBlueLuminosity);
				...
				col.rgb = tex2D(_ColorRampTex, float2(luminance, 0)).rgb;
				#endif
				...
				float3 dif = abs(currChangeColor - _ColorChangeTarget.rgb);
				col.rgb = lerp(col.rgb, float3(luminance, luminance, luminance) * _ColorChangeNewCol.rgb,
					max(sign(1 - saturate(dif.x + dif.y + dif.z) - _ColorChangeTolerance), 0.0));
				#if CHANGECOLOR2_ON
				dif = abs(currChangeColor - _ColorChangeTarget2.rgb);
				col.rgb = lerp(col.rgb, float3(luminance, luminance, luminance) * _ColorChangeNewCol2.rgb,
					max(sign(1 - saturate(dif.x + dif.y + dif.z) - _ColorChangeTolerance2), 0.0));
				#endif
				#if CHANGECOLOR3_ON
				dif = abs(currChangeColor - _ColorChangeTarget3.rgb);
				col.rgb = lerp(col.rgb, float3(luminance, luminance, luminance) * _ColorChangeNewCol3.rgb,
				...
				#if HSV_ON
				float3 resultHsv = float3(col.rgb);
				float cosHsv = _HsvBright * _HsvSaturation * cos(_HsvShift * 3.14159265 / 180);
				float sinHsv = _HsvBright * _HsvSaturation * sin(_HsvShift * 3.14159265 / 180);
				...
				//OUTLINE-------------------------------------------------------------
					...
					#if ATLAS_ON
					i.uvOutDistTex = float2((i.uvOutDistTex.x - _MinXUV) / (_MaxXUV - _MinXUV), (i.uvOutDistTex.y - _MinYUV) / (_MaxYUV - _MinYUV));
					#endif
					...
					#if ATLAS_ON
					i.uvOutTex = float2((i.uvOutTex.x - _MinXUV) / (_MaxXUV - _MinXUV), (i.uvOutTex.y - _MinYUV) / (_MaxYUV - _MinYUV));
					#endif
					float4 tempOutColor = tex2D(_OutlineTex, i.uvOutTex);
					#if OUTGREYTEXTURE_ON
					luminance = 0.3 * tempOutColor.r + 0.59 * tempOutColor.g + 0.11 * tempOutColor.b;
					tempOutColor = float4(luminance, luminance, luminance, 1);
					#endif
				...
				#if FADE_ON
				float2 tiledUvFade1= i.uvFadeTex1;
				float2 tiledUvFade2 = i.uvFadeTex2;
				#if ATLAS_ON
				tiledUvFade1 = float2((tiledUvFade1.x - _MinXUV) / (_MaxXUV - _MinXUV), (tiledUvFade1.y - _MinYUV) / (_MaxYUV - _MinYUV));
				tiledUvFade2 = float2((tiledUvFade2.x - _MinXUV) / (_MaxXUV - _MinXUV), (tiledUvFade2.y - _MinYUV) / (_MaxYUV - _MinYUV));
				#endif
				float fadeTemp = tex2D(_FadeTex, tiledUvFade1).r;
				...
				#if SHADOW_ON
				float shadowA = tex2D(_MainTex, i.uv + float2(_ShadowX, _ShadowY)).a;
				float preMultShadowMask = 1 - (saturate(shadowA - col.a) * (1 - col.a));
				...
				uvShine += float2(0.5, 0.5);
				float shineMask = tex2D(_ShineMask, i.uv).a;
				...
				* max(sign((_ShineLocation + _ShineWidth) - currentDistanceProjection), 0.0) * _ShineColor * shineMask;
				...
				#if HOLOGRAM_ON
				float totalHologram = _HologramStripesAmount + _HologramUnmodAmount;
				float hologramYCoord = ((uvRect.y + (((_Time.x + _RandomSeed) % 1) * _HologramStripesSpeed)) % totalHologram) / totalHologram;
				hologramYCoord = abs(hologramYCoord);
				float alpha = RemapFloat(saturate(hologramYCoord - (_HologramUnmodAmount/totalHologram)), 0.0, 1.0, _HologramMinAlpha, saturate(_HologramMaxAlpha));
				float hologramMask = max(sign((_HologramUnmodAmount/totalHologram) - hologramYCoord), 0.0);
